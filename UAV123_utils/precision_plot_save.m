function precisions = precision_plot_save(positions, ground_truth, title, savedir, show)
...
% 		fprintf('%12s - Number of ground truth frames does not match number of tracked frames.\n', title)
	...
	% plot the precisions
	if show == 1
		han = figure('Name',['Precisions - ' title]);
        set(han,'DefaultFigureVisible','on');
		plot(precisions, 'k-', 'LineWidth',2);
		xlabel('Threshold'), ylabel('Precision');
        saveas(han,[savedir, title,'.png']);
    else
        han = figure('Name',['Precisions - ' title]);
        set(han,'DefaultFigureVisible','off');
		plot(precisions, 'k-', 'LineWidth',2);
		xlabel('Threshold'), ylabel('Precision');
        saveas(han,[savedir, title,'.png']);
	end
